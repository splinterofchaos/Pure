...
template< class X > struct Largest {
    ...
    Largest( X x ) : x(move(x)) { }

    Largest& operator= ( const X& y ) {
        x = std::max( x, y );
    }
};

template< class X > 
Largest<X> largest( X x ) {
    return Largest<X>(move(x));
}
...
void problem4() {
    cout << "The largest palindrome product of three digit numbers :"
         << flush;
    cout << foldl ( 
        []( unsigned int largest, const IRange& r ) -> unsigned int {
            return maybeMaximum( largest, 
                                 filter( palindrome, 
                                         // Note: f ^ xs = map(f,xs).
                                         times(last(r)) ^ init(r) ) );
        },
    ...
    cout << (
        foldl ( 
            []( int x, const vector<int>& v ) {
                return std::max( x, product( take(5,v) ) );
            }, 0, tails( mapTo<vector>(from_sym,nsStr) )
        ) 
    ) << endl;
}
